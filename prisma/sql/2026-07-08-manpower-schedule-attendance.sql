...
--
-- This is a SEPARATE real table, not columns added onto "ManpowerSchedule" —
-- that model is a view backed by a foreign table (FDW into ebright_hrfs; see
-- the ON CONFLICT comment on POST /api/schedules), so ALTER TABLE ADD COLUMN
-- against it fails. A first attempt at this feature tried exactly that and
-- broke every save — this table sidesteps the problem entirely.
