...
const GRADE_COUNT = 8;
const STORAGE_KEY = "SNR-grade-table";
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-amber-700 text-white">
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-amber-800">Grade</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-amber-800">Enrol</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-amber-800">Finished</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-amber-800">Balance</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: GRADE_COUNT }, (_, i) => i + 1).map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      onClick={() => router.push(`/academy/syllabus/snr/${n}`)}
                      className="px-4 py-3 border border-slate-200 text-sm font-semibold text-amber-700 underline-offset-2 hover:underline cursor-pointer"
                    >
                      Grade {n}
                    </td>
                    {(["enrol", "finished", "balance"] as const).map((field) => (
                      ...
                      >
                        <input
                          ...
                          className="w-full px-4 py-3 text-sm text-slate-700 bg-transparent focus:outline-none focus:bg-amber-50"
                        />
                      </td>
                    ))}
                  </tr>
