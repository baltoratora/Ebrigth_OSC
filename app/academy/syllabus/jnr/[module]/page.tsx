...
export default function JnrModulePage() {
  ...
  // Only grades 1–8 are valid for JNR
  if (!Number.isInteger(n) || n < 1 || n > 8) {
    return (
      <div className="w-full max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
          <h1 className="text-2xl font-black text-slate-800 mb-2">Grade not found</h1>
          <p className="text-slate-500 mb-6">JNR has grades 1 through 8.</p>
          <button
            onClick={() => router.push("/academy/syllabus/jnr")}
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 min-h-[60vh]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {chapterNames.map((name, idx) => (
            <div
              key={name}
              onClick={() => router.push(`/academy/syllabus/jnr/${n}/${idx + 1}`)}
              className="bg-green-200 text-green-900 rounded-2xl shadow-md p-8 h-full flex flex-col items-center justify-center text-center transition-transform hover:scale-105 cursor-pointer"
            >
