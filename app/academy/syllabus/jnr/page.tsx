...
const GRADE_COUNT = 8;
const STORAGE_KEY = "JNR-grade-table";
...
export default function JnrPage() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          // Pad / trim to match GRADE_COUNT so adding/removing grades later
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-teal-800 text-white">
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-teal-900">Grade</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-teal-900">Enrol</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-teal-900">Finished</th>
                <th className="text-left px-4 py-3 text-sm font-black uppercase tracking-wide border border-teal-900">Balance</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: GRADE_COUNT }, (_, i) => i + 1).map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      onClick={() => router.push(`/academy/syllabus/jnr/${n}`)}
                      className="px-4 py-3 border border-slate-200 text-sm font-semibold text-teal-800 underline-offset-2 hover:underline cursor-pointer"
                    >
                      Grade {n}
                    </td>
                    {(["enrol", "finished", "balance"] as const).map((field) => (
                      ...
                      >
                        <input
                          ...
                          className="w-full px-4 py-3 text-sm text-slate-700 bg-transparent focus:outline-none focus:bg-teal-50"
                        />
                      </td>
                    ))}
                  </tr>
