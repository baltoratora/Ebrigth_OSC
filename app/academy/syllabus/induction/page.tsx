...
export default function InductionPage() {
  ...
  const label = "INDUCTION";
  const editor = useChapterEditor("INDUCTION-main");
  const [panelOpen, setPanelOpen] = useState(false);

  // Hover-to-open + grace-period close — same pattern as the JNR/MDR/SNR chapter pages.
  ...
  // Top-edge trigger: bring up the panel when cursor approaches the top 30px.
  useEffect(() => {
    const TOP_PX = 30;
  ...
  // Pagination: count how many A4 pages the section currently spans.
    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-4 mb-8">
        <button
          ...
          className="bg-slate-100 text-slate-700 hover:bg-indigo-700 hover:text-white px-4 py-2 rounded-xl flex items-center justify-center shadow-md transition-colors"
        ...
        <div className="bg-indigo-700 text-white px-4 py-2 rounded-xl flex items-center gap-2 shadow-md">
      ...
      {/* Editable A4 page with page-break band, header/footer/page-number bands */}
      <section
        ref={sectionRef}
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mt-6">
          <button
            ...
            title="Save"
            onClick={() => editor.save()}
            className="p-2 rounded-full bg-indigo-200 hover:bg-indigo-300 text-indigo-900 border border-indigo-300 hover:border-indigo-500 transition-colors"
          >
